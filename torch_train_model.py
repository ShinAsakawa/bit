    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['val', 'train']:
            ...
            for inputs, labels in tqdm(dataloaders_dict[phase]):
                optimizer.zero_grad()  # optimizer の初期化
                labels0 = labels[0]
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':  # 訓練時は誤差逆伝播による学習を実行
                        loss.backward()
                        optimizer.step()

                    epoch_loss += loss.item() * inputs.size(0)         # 損失値の合計を更新
                    epoch_corrects += torch.sum(preds == labels0.data) # 正解数の合計を更新
